...
from model import BiLSTM_CRF_DAE
from tool import tool, logger
from openpyxl import load_workbook
from model_dae import TransformerEncoderModel, BiLSTM_CRF, CNN_CRF, BiLSTM_CRF_ATT
...
class EE():
    ...
    def train(self):
        ...
        elif config.model_name == 'BiLSTM_CRF_DAE':
            model = BiLSTM_CRF_DAE(config, ntoken=len(self.word_vocab), ntag=len(self.tag_vocab), vectors=vectors).to(device)
    ...
    def eval(self, dev_iter):
        ...
        for index, iter in enumerate(tqdm(dev_iter)):
            ...
            for i, result_list in zip(range(text.size(1)), result):
                ...
                for i, entity in enumerate(['origin_place','size', 'transfered_place']):
                    entities[entity]['p'] = entities[entity]['TP'] / entities[entity]['S'] \
                        if entities[entity]['S'] != 0 else 0
                    entities[entity]['r'] = entities[entity]['TP'] / entities[entity]['G'] \
                        if entities[entity]['G'] != 0 else 0
                    f1 = 2 * entities[entity]['p'] * entities[entity]['r'] / \
                                                   (entities[entity]['p'] + entities[entity]['r']) \
                        if entities[entity]['p'] + entities[entity]['r'] != 0 else 0
                    f[i]+=f1
                ...
                sum1 += 1
        print('test：origin_place：{}  size：{}  transfered_place：{}  totle：{}'.format(f[0]/sum1,f[1]/sum1,f[2]/sum1,sum(f)/(3*sum1)))
        TP = 0
        S = 0
        G = 0
        print('\n--------------------------------------------------')
    ...
    def predict_test(self, path=config.test_path, model_name=config.model_path.format(config.experiment_name),  save_path=config.unformated_val_path.format(config.experiment_name)):
        ...
        for line_num in tqdm(range(max_row-1)):
            ...
            text = torch.tensor(numpy.array([word_vocab.stoi[word] for word in sentence], dtype='int64')).unsqueeze(1).expand(len(sentence),config.batch_size).to(device)
            text_len = torch.tensor(numpy.array([text.size(0)], dtype='int64')).expand(config.batch_size).to(device)
            result = model(text, text_len)[0]
            tag_pred = []
            list1 = [tag_vocab.itos[k] for k in result]
            ...
            while i < len(tag_pred):
                ...
                i+=1
            ws.cell(line_num, 2).value = ','.join(origin_places)
            ws.cell(line_num, 3).value = ','.join(sizes)
            ws.cell(line_num, 4).value = ','.join(transfered_places)
